...
class TwoDManagementController extends GetxController {
  //controllers
  ...
  //services
  final RulesService twoDRulesService = RulesService();
      ...
      onConfirmBtnTap: () {
        for (String numberStr in _selectedNumbers) {
          addedBets.add({
            ...
            'number': numberStr,
            'maxAmount': int.parse(amountController.text),
          });
        }
  ...
  Future<void> limit() async {
    ...
    try {
      final res = await twoDRulesService.setRules(
        addedBets,
        pattern.value,
        session,
        'TWO_D',
