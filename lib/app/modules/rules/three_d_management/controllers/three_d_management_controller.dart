...
import 'package:winner_v3_5/app/modules/rules/two_d_managment/controllers/rules_service.dart';

class ThreeDManagementController extends GetxController {
  //controllers
  ...
  //services
  final RulesService threeDRulesService = RulesService();
        ...
        for (String numberStr in selectedNumbers) {
          addedBets.add({
            'id':
                ...
                numberStr, // Unique ID for the bet
            'number': numberStr, // Individual number string
            'maxAmount': int.parse(
  ...
  void updateBetAmount(Map<String, dynamic> betEntry, int newAmount) {
    ...
    if (index != -1) {
      addedBets[index]['maxAmount'] = newAmount;
  ...
  Future<void> limit() async {
    ...
    try {
      final res = await threeDRulesService.setRules(
        addedBets,
        pattern.value,
        null,
        'THREE_D',
